...
TLSPretense provides a test framework for testing SSL certificate
validation. It generates a set of certificates containing specific flaws, and
it presents the certificates to a client that has been configured to trust a CA
used by TLSPretense. The test framework then configures its system's firewall
to redirect and intercept network traffic so that the test runner can present
its certificate to the client. To speed up testing, the test runner starts the
next test as soon as the current test finishes.
...
* A Unix-like system that uses a supported firewall/routing implementation.
  TLSPretense (through the included PacketThief gem) currently supports:
  * Netfilter on Linux
  * IPFW on MacOSX and *BSD (less well tested)
...
== Contributing to SSL Client Testing
